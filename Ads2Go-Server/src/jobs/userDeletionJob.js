    ...
    try {
      ...
      for (const ad of adsToDelete) {
        try {
          ...
          // 2.5. Remove ad data from UserAnalytics (detailed analytics)
          try {
            ...
            const userAnalyticsDocs = await UserAnalytics.find({
              $or: [
                { 'ads.adId': adId },
                { 'dailyStats.ads.adId': adId }
              ]
            });

            console.log(`   📊 Found ${userAnalyticsDocs.length} UserAnalytics documents containing ad ${adId}`);
            ...
            for (const userAnalytics of userAnalyticsDocs) {
              ...
              userAnalytics.ads = userAnalytics.ads.filter(
                ad => ad.adId.toString() !== adId.toString()  // Only removes the specific deleted ad
              );
              ...
              if (userAnalytics.ads.length < initialAdsCount) {
                ...
                console.log(`      ✅ All other ads preserved: ${adIdsBefore.filter(id => id !== adId.toString()).length} ads`);
              }
                ...
                for (const dailyStat of userAnalytics.dailyStats) {
                  if (dailyStat.ads && Array.isArray(dailyStat.ads)) {
                    ...
                    dailyStat.ads = dailyStat.ads.filter(
                      ad => ad.adId.toString() !== adId.toString()
                    );
            ...
            console.warn(`⚠️ Warning: Could not remove ad from UserAnalytics:`, userAnalyticsError.message);
          }
